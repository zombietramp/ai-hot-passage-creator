...
import ai.shizhongying.template.annotation.AgentExecution;
import ai.shizhongying.template.constant.PromptConstant;
import ai.shizhongying.template.model.dto.article.ArticleState;
import ai.shizhongying.template.model.dto.image.ImageRequest;
...
public class ArticleAgentService {
    ...
    public void agent3GenerateContent(ArticleState state, Consumer<String> streamHandler) {
        String outlineText = JSON.toJSONString(state.getOutline().getSections());
    ...
    public void agent5GenerateImages(ArticleState state, Consumer<String> streamHandler) {
        ...
        for (ArticleState.ImageRequirement requirement : state.getImageRequirements()) {
            ...
            // 推送单张配图完成
            String imageCompleteMessage = SseMessageTypeEnum.IMAGE_COMPLETE.getStreamingPrefix() + JSON.toJSONString(imageResult);
