...
import ai.shizhongying.template.config.CosConfig;
import ai.shizhongying.template.config.StorageConfig;
import ai.shizhongying.template.model.dto.image.ImageData;
...
public class CosService {
    ...
    public void init() {
        if (storageConfig.isLocal()) {
            log.info("存储模式: local, 跳过 COSClient 初始化, 本地目录: {}", storageConfig.getLocal().getDir());
            return;
        }
        ...
        log.info("存储模式: cos, bucket={}, region={}", cosConfig.getBucket(), cosConfig.getRegion());
    }
    ...
    public String uploadBytes(byte[] bytes, String mimeType, String folder) {
        ...
        if (storageConfig.isLocal()) {
            return saveBytesToLocal(bytes, fileName);
        }
    ...
    public String uploadFile(File file, String folder) {
        try {
            ...
            if (storageConfig.isLocal()) {
                return saveBytesToLocal(fileBytes, fileName);
            }
     ...
     * 保存字节数据到本地
     */
    private String saveBytesToLocal(byte[] bytes, String fileName) {
        try {
            StorageConfig.Local local = storageConfig.getLocal();
            Path target = Paths.get(local.getDir(), fileName).toAbsolutePath().normalize();
            ...
            String url = trimTrailingSlash(local.getUrlPrefix()) + "/" + fileName.replace(File.separatorChar, '/');
            log.info("文件已保存到本地, path={}, url={}", target, url);
            return url;
        } catch (IOException e) {
            log.error("保存字节数据到本地失败, fileName={}", fileName, e);
            return null;
        }
    }
